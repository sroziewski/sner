...
def plot_hist(_df, _column):
    import matplotlib.pyplot as plt

    # An "interface" to matplotlib.axes.Axes.hist() method

    _pos_df = get_rows(_df, 'notified', 1)
    _neg_df = get_rows(_df, 'notified', 0)

    _pos_val = _pos_df[_column].tolist()
    _neg_val = _neg_df[_column].tolist()

    n, bins, patches = plt.hist(x=_neg_val, bins='auto', color='#123123',
                                alpha=0.7, rwidth=0.85)

    n2, bins2, patches2 = plt.hist(x=_pos_val, bins='auto', color='#0504aa',
                                alpha=0.7, rwidth=0.85)

    print("column: {} n_p: {} n_n: {} bins_p: {} bins_n: {} patches_p: {} patches_n: {}".format(_column, len(n2), len(n), len(bins2), len(bins), len(patches2), len(patches)))
...
columns_to_drop = ['alert_ids', 'start_minute', 'timestamp_dist', 'ip', 'correlatedcount', 'srcip_cd', 'srcport_cd', 'dstport_cd', 'thrcnt_month', 'thrcnt_week', 'thrcnt_day']
...
for column in data_training_raw.columns.tolist():
    plot_hist(data_training_raw, column)
    # print("column: {} : {}".format(column, len(data_training_raw.groupby(column).groups)))
